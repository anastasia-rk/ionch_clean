...
# set the exponents
lambda_exps = [2,3,4,5,6,7]  # gradient matching weight - test
nIters = 500
...
if __name__ == '__main__':
    ...
    df_best = pd.DataFrame(columns=['lambda', '$L(B / \hat{\Theta}^{*}, Y)$', '$L_{data}(\hat{\Theta}^{*} / B, Y)$', '$L_{ODE}(B / \hat{\Theta}^{*}, Y) $'])
        ...
        if PlotParams:
            ...
            for iAx, ax in enumerate(axes2.flatten()):
                for iIter in range(len(theta_visited)):
                    ...
                    ax.scatter(iIter * np.ones(len(x_visited_iter)), x_visited_iter, c=colours[counter], marker='.', alpha=0.075,
                               linewidth=0)
                ...
                ax.text(nIters, np.exp(theta_best[-1, iAx]), "{:.6f}".format(np.exp(theta_best[-1, iAx])), fontsize=8)

            # plot parameter values converting from log scale to decimal
            for iAx, ax in enumerate(axes3.flatten()):
                for iIter in range(len(theta_visited)):
                    x_visited_iter = [theta_visited[iIter][i][iAx] for i in range(len(theta_visited[iIter]))]
                    ax.scatter(iIter * np.ones(len(x_visited_iter)), np.exp(x_visited_iter), c=colours[counter], marker='.', alpha=0.075,
                               linewidth=0)
                ax.plot(np.exp(theta_best[:, iAx]), '-',color = colours[counter], linewidth=1,
                        label=r'$\lambda$ = ' +  "{:.2e}".format(lambd) +r", $" + param_names[iAx] + " = $" + "{:.6f}".format(np.exp(theta_best[-1, iAx])))
                # at the right side of ax, add text at the height of the last value wih the value
                ax.text(nIters, np.exp(theta_best[-1, iAx]), "{:.6f}".format(np.exp(theta_best[-1, iAx])), fontsize=8)
        # ####################################################################################################################
        if PlotCosts:
            # fill in a row of the dataframe
            df_best.loc[counter] = [lambd, f_inner_best[-1], f_outer_best[-1], f_gradient_best[-1]]
            ## plot evolution of inner costs
            for iIter in range(len(f_outer_best)):
                ax4.scatter(iIter * np.ones(len(InnerCosts_all[iIter])), InnerCosts_all[iIter], c=colours[counter],
                            marker='.', alpha=0.075, linewidths=0)
            ax4.plot(f_inner_best, '-', color=colours[counter], linewidth=1,
                     label=r'$\lambda$ = ' +  "{:.2e}".format(lambd) +', '  + r'$L(B / \hat{\Theta}^{*}, Y) = $' + "{:.5e}".format(f_inner_best[-1]))
            #
            # plot evolution of outer costs
            for iIter in range(len(f_outer_best)):
                ax5.scatter(iIter * np.ones(len(OuterCosts_all[iIter])), OuterCosts_all[iIter], c=colours[counter], marker='.',
                            alpha=0.075, linewidths=0)
            ax5.plot(f_outer_best, '-', color=colours[counter], linewidth=1,
                     label=r'$\lambda$ = ' +  "{:.2e}".format(lambd) +', ' + r'$L_{data}(\hat{\Theta}^{*} / B, Y) = $' + "{:.5e}".format(f_outer_best[-1]))
            #
            # plot evolution of gradient costs
            for iIter in range(len(f_gradient_best)):
                ax6.scatter(iIter * np.ones(len(GradCosts_all[iIter])), GradCosts_all[iIter], c=colours[counter], marker='.',
                            alpha=0.075,linewidths=0)
            ax6.plot(f_gradient_best, '-',  color=colours[counter], linewidth=1,
                     label=r'$\lambda$ = ' +  "{:.2e}".format(lambd) +', ' + r'$L_{ODE}(B / \hat{\Theta}^{*}, Y) = $' + "{:.5e}".format(f_gradient_best[-1]))
        #################################################################################################################
        # done plotting
        counter += 1
    ...
    if PlotCosts:
        ...
        sns.heatmap(df_best, annot=True, fmt=".2e", cmap='magma', ax=ax,cbar=False)
