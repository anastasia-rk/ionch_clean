...
# set the exponents
lambda_exps = [1,2,3,4,5,6,7,8]  # gradient matching weight - test
...
if __name__ == '__main__':
    ...
    # times_for_validation = times_ap
    # load_protocols.volts_interpolated = volts_interpolated_ap
    # for fitting results
    times_for_validation = times
    load_protocols.volts_interpolated = volts_interpolated_for_plots

    voltage_for_validation = V(times_for_validation)
    solution, current_model = generate_synthetic_data(model_name, thetas_true, times_for_validation)
    current_true_noiseless = current_model(times_for_validation, solution, thetas_true)
    ...
    # axes8[-1].plot(times_for_validation, voltage_for_validation, '-k', label='Voltage', linewidth=1.5, alpha=0.27)
    # axes8[-2].plot(times_for_validation, current_true_noiseless, '-k', label='True current', linewidth=1.5, alpha=0.27)
    # for iState in range(len(y_labels8) - 2):
    #     axes8[iState].plot(times_for_validation, solution.sol(times_for_validation)[iState, :], '-k', label='True state', linewidth=1.5,
    #                        alpha=0.27)
    # # save
    # fig8.tight_layout()
    # fig8.savefig(folderForOutput + '/' + model_name.lower() + '_check_on_ap_protocol.png', dpi=400)

    axes7[0].plot(times_for_validation, current_true_noiseless, '-k', label=r'True current', linewidth=2, alpha=0.27)
    ...
    for weight in tqdm(lambda_exps):
        ...
        if Validation:
            ...
            # compute model with validation protocol
            load_protocols.volts_interpolated = volts_interpolated_for_plots
            voltage_ap = V(times_for_validation)
            solution, current_model = generate_synthetic_data(fitted_model_name, Thetas_ODE, times_for_validation)
            states_optimised_ODE = solution.sol(times_for_validation)
            current_ODE_output = current_model(times_for_validation, solution, Thetas_ODE)
            current_residual = current_ODE_output - current_true_noiseless
            current_rmse = np.sqrt(np.mean(current_residual** 2))
            # plot the current generated by the fitted model on validation protocol
            axes7[0].plot(times_for_validation, current_ODE_output, '--', color=colours[counter],
                          label=r'$\lambda = 10^{' + str(weight) + '}$',
                          linewidth=1, alpha=0.5)
            axes7[1].plot(times_for_validation, current_residual, '--', color=colours[counter],
                          linewidth=1, alpha=0.5)
            # add the RMSE to the plot of lambda vs RMSE
            # axes7[0].text(times_for_validation[-1], current_ODE_output[-1], "{:.2f}".format(current_rmse), fontsize=8)
    ...
    if Validation:
        ...
        current_rmse = np.sqrt(np.mean(current_residual ** 2))
        # plot the current generated by the fitted model on validation protocol
        axes7[0].plot(times_for_validation, current_ODE_output, '--', color='green',
                      label=r'$\lambda = 10^{' + str(weight) + '}$',
                      linewidth=0.8, alpha=0.3)
        axes7[1].plot(times_for_validation, current_residual, '--', color='green',
                      linewidth=0.8, alpha=0.3)
        # add the RMSE as a dashed line to mark the reference value
        axes7[-1].plot([lambda_exps[0], lambda_exps[-1]], [current_rmse,current_rmse], '--', color='green', linewidth=1, alpha=0.3, label='IVP solver')
        ############################################################################################################
        ...
        for ax in axes7:
            ...
            ax.grid(which='major', color='grey', linestyle='solid', alpha=0.2, linewidth=1)
            iAx += 1
        if model_name.lower() == 'wang':
            axes7[0].set_ylim(-3, 3)
            axes7[1].set_ylim(-3, 3)
        elif model_name.lower() == 'kemp':
            axes7[0].set_ylim(-3, 3)
            axes7[1].set_ylim(-3, 3)
        axes7[0].set_xlim(times_for_validation[0], times_for_validation[-1])

        axes7[1].set_xlim(times_for_validation[0], times_for_validation[-1])
        axes7[1].set_xlabel('Time, ms')
        axes7[-1].set_xlabel(r'$\log(\lambda)$')
        axes7[-1].set_ylabel('RMSE')
        axes7[-1].set_yscale('log')
        axes7[-1].legend(fontsize=12, loc='lower right')
        for ax in axes7.flat:
            ax.yaxis.set_label_coords(-0.05, 0.5)
        fig7.tight_layout()
        fig7.savefig(folderForOutput + '/fitting_results.png', dpi=600)
    # end of if statement
