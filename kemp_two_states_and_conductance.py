...
class BoundariesTwoStates(pints.Boundaries):
    ...
    def __init__(self):
        ...
        self.upper_conductance = self.lower_conductance*1000 # 10
...
if __name__ == '__main__':
    ...
    lambd = 10e5  # 0.3 # 0 # 1 ## - found out that with multiple states a cost with lambda 1 does not cope for segments where a is almost flat
    ...
    EK = -80
    thetas_true = [2.07e-3, 7.17e-2, 3.44e-5, 6.18e-2, 4.18e-1, 2.58e-2, 4.57e-2, 2.51e-2, 3.33e-2]
    theta_true = np.log(thetas_true)
    ...
    theta_lower_boundary = [np.log(10e-5), np.log(10e-5), np.log(10e-5), np.log(10e-5), np.log(110e-5), np.log(10e-5), np.log(10e-5), np.log(10e-5), np.log(10e-3)]
    theta_upper_boundary = [np.log(10e3), np.log(0.4), np.log(10e3), np.log(0.4), np.log(10e3), np.log(0.4), np.log(10e3), np.log(0.4), np.log(10)]
    ################################################################################################################
    ...
    class OuterCriterion(pints.ProblemErrorMeasure):
        ...
        def __call__(self, theta):
            ...
            *params, conductance = theta[:9]
            ...
            # compute the data fit
            d_y = conductance * np.prod(x, axis=0) * (self._values[:,1] - EK) - self._values[:,0] # this part depends on theta_g
            data_fit_cost = np.transpose(d_y) @ d_y
    ...
    result_at_HH_37 = inner_optimisation(Thetas_ODE,times_roi,voltage_roi,current_roi,knots_roi,states_known_roi,init_betas_roi)
    betas_sample, inner_costs_sample, state_fitted_roi = result_at_HH_37
    ...
    column_names = ['Iteration', 'Walker', 'Theta_1', 'Theta_2', 'Theta_3', 'Theta_4','Theta_5', 'Theta_6', 'Theta_7', 'Theta_8', 'Theta_9', 'Inner Cost', 'Outer Cost',
                    'Gradient Cost']
    ...
    ####################################################################################################################
    # plot model outputs given the best theta
