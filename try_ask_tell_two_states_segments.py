...
if __name__ == '__main__':
    ...
    axes['a)'].plot(times, current_model, '--c', label=r'Optimised given true $\theta$')
    axes['b)'].plot(times, solution.sol(times)[0, :], '-k', label=r'$a$ true', linewidth=2, alpha=0.7)
    axes['b)'].plot(times, state_fitted_roi[state_names[0]], '--c', label=r'B-spline approximation given true $\theta$')
    axes['c)'].plot(times, solution.sol(times)[1, :], '-k', label=r'$r$ true', linewidth=2, alpha=0.7)
    axes['c)'].plot(times, state_fitted_roi[state_names[1]], '--c', label=r'B-spline approximation given true $\theta$')
    iAx = 0
